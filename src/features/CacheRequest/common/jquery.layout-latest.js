	...
	*/
	...
	var onResizerLeave	= function (evt, el) {
		...
		if (!el) { // 1st call - mouseleave event
			...
			timer.set(name, function(){ this.onResizerLeave(evt, e); }, 200);
		}
 ...
 */
	...
	*/
	...
	var slideToggle = function (pane) { this.toggle(pane, true); };
	...
	*/
	var bindStopSlidingEvents = function (pane, enable) {
		...
		if (!enable)
			...
			$R.css("cursor", o.sliderCursor );
			$R.attr("title", o.togglerTip_open ); // use Toggler-tip, eg: "Close Pane"
		}
	...
	*/
	...
	var enableClosable = function (pane) {
		...
		if (!$T) return;
		o.closable = true;
		$T	.bind("click."+ sID, function(evt){ this.toggle(pane); evt.stopPropagation(); })
			.bind("mouseenter."+ sID, addHover)
			.bind("mouseleave."+ sID, removeHover)
 ...
 */
	...
	*/
	function allowOverflow (el) {
		...
		if (s.cssSaved)
			this.resetOverflow(pane); // reset previous CSS before continuing
		...
		$.each(_c.allPanes.split(","), function(i, p) {
			if (p != pane) this.resetOverflow(p);
		});
